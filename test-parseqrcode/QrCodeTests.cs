...
namespace test_parseqrcode
{
	public class QrCodeTests
	{
		...
		private const string DefaultJWSHeader = "eyJhbGciOiJFUzI1NiJ9"; // Seite 42 der Spezifikation - Header Base64 URL encoded
		...
		public void CertPlayGround()
		{
			...
			using (ECDsa ecdsa = cert.GetECDsaPublicKey())
			{
				if (ecdsa != null)
				{
					...
					{
						string dataUrlEncoded = UrlBase64.Encode(System.Text.Encoding.UTF8.GetBytes(test.GetDataForHashing()));
						string dataForHashing = $"{DefaultJWSHeader}.{dataUrlEncoded}";

						byte[] data = System.Text.Encoding.UTF8.GetBytes(dataForHashing);
